...
1. **分页获取所有相关仓库**，用于识别用户活跃的仓库范围。**Gitee 和 GitCode 的仓库发现策略不同**：
   ...
   **Gitee PR 获取**（对去重后的每个仓库）：
   ```bash
   # 企业版仓库：MCP list_repo_pulls（author 过滤，state=all）
   # 社区版仓库：REST API /repos/{repo}/pulls?state=all
...
6. **subagent 并行化（⭐ 最大加速点，优先方案）**：

   阶段3是整个流程的瓶颈（每个 PR 需要 2-4 次 API 调用 + patch 解析），且 PR 之间完全独立、无共享状态，非常适合 subagent 并行化。

   **方案**：将 PR 列表按数量分片（每 15-20 个 PR 一个 subagent），每个 subagent 独立完成：
   1. 对其负责的 PR 调用 diff API（GitCode 三步：详情 → compare → files 补全；Gitee 社区版一步；Gitee 企业版 MCP `get_diff_files`）
   2. 对 patch 做截断处理（前 500 行）
   3. 将分析结果写入 `02_intermediate/pr_diffs_batch_{i}.json`
   4. 主 agent 等待全部 subagent 完成 → 合并所有 batch JSON 为 `pr_diffs.json`

   ```
   串行流程（慢）:
     遍历所有 PR: 获取diff → 解析patch → 写入pr_diffs.json
     → 耗时 = PR数 × (网络延迟 + 分析耗时)

   并行流程（快）:
     将PR列表分成N个batch（每batch 15-20个PR）
     同时启动N个subagent:
       每个subagent: 对batch内PR获取diff+截断 → 写入pr_diffs_batch_{i}.json
     主agent合并 → pr_diffs.json
     → 耗时 ≈ max(各batch耗时) ≈ 总耗时 / N
   ```
...
| Diff 获取 | MCP 工具 `get_diff_files`（返回 files[]，patch 为字符串，类似社区版。**注意**：返回的 additions/deletions 字段为字符串类型如 `"10"`，需 `int()` 转换后再累加） |
...
10. **subagent 加速策略总结**：各阶段 subagent 并行化收益不同，按下表优先级应用：
    ...
    |--------|------|----------|-----------|------|
    | **P0** | 阶段3 Diff 分析 | PR 分片（每 15-20 个一个 subagent），并行获取 diff + patch 截断 | 3-5x | 收益最大，diff 获取是瓶颈，PR 间完全独立 |
