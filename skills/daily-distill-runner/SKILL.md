...
本 Skill 的身份、类型约束与询问规则详见 [skill 类型与协作工作流](../../docs/skill-types-and-workflows.md)。核心职责：
1. **确定主 skill**：询问用户每晚 22:00 要运行哪个主 skill，记录后才进入等待
2. **定时等待**：阻塞至每晚 22:00（纯 bash sleep，不消耗 AI token）
3. **执行主 skill**：22:00 到达后，用 skill 工具加载并执行主 skill
4. **记录并继续**：任务结束后记录摘要，不打断对话，回到等待状态
5. **无限循环**：次日 22:00 再次触发，周而复始

> 本 Skill 只负责"准点触发 + 循环"。主 skill 的具体内容由用户在激活时指定，本 Skill 不关心主 skill 的内部细节。
