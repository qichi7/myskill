...
本 skill 的身份、类型约束与询问规则详见 [skill 类型与协作工作流](../../docs/skill-types-and-workflows.md)。通过「启动检测」完成前置检查；待主 skill 启动时激活监控，在其执行期间持续守护。
...
1. **询问主 skill**：本 skill 被激活后，第一件事是询问用户"要守护的主 skill 是什么？"。按 [skill 类型与协作工作流](../../docs/skill-types-and-workflows.md) 中定义的辅助 skill 询问规则执行。

2. 运行启动检测：
   ```bash
   bash scripts/check_guard.sh init
   ```
3. 如果返回 `STOP`：当前 provider 非 Cannbot，**告知用户并拒绝继续**，不得进入阶段二。
4. 如果返回 `ARMED`：告知用户"守卫已就绪，请在主 skill 启动时告知我"，然后**等待**。此时不得运行 `check` 或 `activate`。
5. **不要在阶段一运行 `activate` 或 `check`**。等待用户启动主 skill。

### 阶段二：任务监控

6. 当用户开始执行主 skill 时，**第一步**运行：
   ```bash
   bash scripts/check_guard.sh activate
   ```
7. 如果 `activate` 返回 `STOP`：不得执行主 skill 的任何内容。
8. `activate` 返回 `OK` 后，在主 skill 的**每个主要步骤/迭代之前**运行：
   ```bash
...
10. **不跳过检查**：即使上一次返回 OK，下一步前仍需检查。脚本内部已处理间隔控制。
11. **停止即终止**：一旦收到 STOP，不得继续任何后续步骤。
12. 如果用户在阶段一之前直接给任务指令，应先提醒用户运行「启动检测」。
